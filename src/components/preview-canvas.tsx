...
export default function PreviewCanvas({ videoRef }: PreviewCanvasProps) {
	...
	useEffect(() => {
		...
		const isDefaultLightBg =
			backgroundColor === gradients[1].gradient &&
	...
	// Stage background from store only (concrete angles — no :root CSS variable tricks)
	const displayBackground = useMemo(
		() =>
			previewStageBackgroundStyle({
				backgroundType,
				backgroundColor,
				gradientAngle,
				imageBackground,
			}),
		[backgroundType, backgroundColor, gradientAngle, imageBackground],
	);
	...
	return (
		<div className="w-full h-full flex items-center justify-center">
			{/* Canvas Background */}
			<div
				data-export-preview-outer
				className={cn(
					"rounded-lg transition-colors duration-300 flex items-center justify-center shadow-lg max-w-4xl w-full h-full",
				...
				>
					...
					>
						...
						{isUploading && effectiveVideoSrc && (
							<div
								className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 dark:bg-black/70 backdrop-blur-sm"
