...
import { QUALITY_OPTIONS } from "@/lib/types/video"
...
export default function ExportModule({ aspectRatio, onClose }: ExportModuleProps) {
  ...
  const footer = (
    <div className="flex gap-2">
      ...
      <button className="flex-1 px-3 py-2 rounded bg-accent text-accent-foreground hover:bg-accent/90 transition-colors text-xs font-medium flex items-center justify-center gap-1.5">
        <Download size={14} />
        Export
      </button>
    </div>
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title="Export Video" footer={footer} maxWidth="max-w-sm">
