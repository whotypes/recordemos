...
import { AnimatePresence, motion, Transition, useInView, Variants } from "framer-motion";
import { AlertCircle, ArrowLeft, ArrowRight, Eye, EyeOff, Gem, Loader, Lock, Mail, X } from "lucide-react";
...
const DiscordIcon = (props: React.SVGProps<SVGSVGElement>) => (
    <svg {...props} viewBox="0 0 256 199" preserveAspectRatio="xMidYMid" className="w-6 h-6">
        <path
...
const modalSteps = [
    ...
    { message: "Welcome back!", icon: <Gem className="w-12 h-12 text-green-500" /> }

];

const TEXT_LOOP_INTERVAL = 1.5;

const DefaultLogo = () => (<img src="/rd_logo.png" alt="Record Demos" className="size-16 dark:brightness-75 before" />);
...
export const SignInComponent = ({ logo = <DefaultLogo />, brandName = "RecordDemos" }: SignInComponentProps) => {

    const { signIn, isLoaded, setActive } = useSignIn();

    const signInWith = (strategy: 'oauth_google' | 'oauth_github' | 'oauth_discord') => {
    ...
    const handleFinalSubmit = async (e: React.FormEvent) => {
        ...
        if (modalStatus !== 'closed' || authStep !== 'password' || !isLoaded) return;
        ...
        try {
            ...
            setModalErrorMessage(err.errors?.[0]?.message || 'Something went wrong. Please try again.');
                ...
                <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-[280px] mx-auto p-4">
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {authStep === "email" && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-4">
                            ...
                            <BlurFade delay={0} className="w-full flex justify-center"><div className="flex items-center gap-2">{logo}<h1 className="text-base font-bold text-foreground">{brandName}</h1></div></BlurFade>

                            <BlurFade delay={0.25 * 1} className="w-full"><div className="text-center"><p className="font-light text-4xl sm:text-5xl md:text-6xl tracking-tight text-foreground whitespace-nowrap">Welcome back</p></div></BlurFade>

                            <BlurFade delay={0.25 * 2}><p className="text-sm font-light text-muted-foreground">Continue with</p></BlurFade>
                        ...
                        {authStep === "password" && <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                            ...
                            <BlurFade delay={0.25 * 1} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-foreground whitespace-nowrap">Enter your password</p></div></BlurFade>

                            <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-muted-foreground">Welcome back! Please enter your password to continue.</p></BlurFade>
