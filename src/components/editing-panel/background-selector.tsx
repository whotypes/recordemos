...
export default function BackgroundSelector() {
	...
	const fetchUnsplashPatterns = async (
		page: number,
	...
	const handleGradientClick = useCallback(
		(gradient: Gradient, isMesh: boolean) => {
			// Full CSS stack in Zustand — preview + export read only the store (no :root vars).
			const bg = isMesh
				? `${gradient.background ?? "transparent"}, ${gradient.gradient}`
				: gradient.gradient;
	...
	const paginatedGradients = gradients.slice(
		startGradientIndex,
		endGradientIndex,
	);
						...
						<>
							<ul className="mt-4 grid grid-cols-4 gap-3 md:grid-cols-5">
								{unsplashData.map((data: UnsplashPhoto) => {
									const imageUrl = highResBackground
										? data.urls.full
										: data.urls.regular;
							...
							return (
								<li key={gradient.gradient} className="h-16 w-16 rounded-md">
									<button
										...
										onClick={() =>
											handleGradientClick(gradient, gradient.type === "Mesh")
										}
										style={
					...
					<div className="flex justify-center">
						<Popover>
							...
							<PopoverContent className="w-64">
								<HexColorPicker
									color={
										backgroundColor.startsWith("#") ? backgroundColor : "#fff"
									}
						...
						{solidColors.map(({ background: solidBackground }) => {
							...
							return (
								<Button
									...
									className={`size-[1.85rem] overflow-hidden rounded-[0.25rem] p-px ${
								...
								>
									{isTransparent ? <X className="h-4 w-4 text-accent" /> : null}
								</Button>
							);
						})}
					</div>
